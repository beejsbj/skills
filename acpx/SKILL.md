...
On bjslab, run `bjslab-context agents` before choosing a route. It resolves the live Codex/OpenCodex, OpenCode, Hermes, K3, DeepSeek, Terra, and Luna handles. A model appearing in the Codex catalog does not mean the active native `spawn_agent` tool accepts it as an override; use that tool's advertised allowlist or launch a native CLI worker.
